...
function LoadHomePageGames(all_game, i, history_id) {
    ...
    $.each(all_game, function(i, field) {
      ...
      $(".home_game_list_all").append(`
                <div class="home_game_box home_game_box_all">
                    <div class="box_a">
                        <img src=${API_SERVER_ADDRESS}/up_img/` + field.img +
        `.webp?from=homepage_allgames" class="game_img" onclick="CheckandPopupServerList(` +
        field.id + `)" gameimg="` + field.id +
        `">
                        ...
                        <div class="top">
                            <div class="icon">
                                ...
                                <i class="layui-icon layui-icon-rate" title="置顶"></i> 
                                -->
                                <i class="layui-icon layui-icon-error" title="删除" onclick="deleteHistoryGame(` +
        field
        .id + `)"></i> 
                            </div>
                        </div>
                        <div class="bottom" onclick="CheckandPopupServerList(` +
        field.id +
        `)">
                            <p>立即加速 <i class="layui-icon layui-icon-next"></i> </p>
                        </div>
                        ...
                        <!-- 加速中效果 -->
                        <div class="game_starting_shadow" game_now_starting_id="` +
        field
  ...
  function LoadGameHistory() {
    ...
    console.log("历史游戏", getLocalHistoryGames(), getLocalHistoryGames()
      .length); // 获取请求参数
    console.log("历史游戏首页缺少", home_game_list_max - getLocalHistoryGames()
      .length); // 获取请求参数
    // 装载历史游戏
  
    $.each(getLocalHistoryGames(), function(i, field) {
      if (!allGamesList.some(item => item.id === field.id + "")) {
      ...
      if (home_game_list_max > i) { // 不够四个游戏或前4个的时候
        LoadHomePageGames(allGamesList, Infinity, field.id);
      }
      LoadHomePageAllGames(allGamesList, field.id);
    })
    // 只有历史游戏是5的时候创建一个隐藏的高 为什么？不然会划不下去
